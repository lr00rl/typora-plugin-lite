...
顶格输入一个 `/` 就切到路径模式，像 Finder 的“前往文件夹”或 shell 提示符那样按路径走：

- `/` 是当前打开的那个文件夹的根
- `//` 是文件系统的根
- `~/` 是主目录

最后一个 `/` 后面的片段还不算路径，它是上一级列表的前缀过滤，所以输入和补全是一个动作而不是两个模式。`Tab` 按所有候选的公共前缀前进：只有一个候选就补全到底，有多个就补到分叉处，不会替你猜。`Enter` 进目录或打开文件，`Backspace`（光标在末尾时）回上一级，`Esc` 关闭。删掉那个 `/` 就回到普通搜索，没有需要退出的模式状态。

目录在前、文件在后，按名称不分大小写排序。点开头的条目默认隐藏，输入 `.` 才出现，隐藏了多少条会写在页脚。目录项数超过上限时页脚会写明只显示了前多少条，不会静默截断。

在「目录」标签页里也能用：那里输入框被写进了 `type:folder scope:…`，触发判断只看你自己敲的那部分，所以斜线不在第 0 位也照样进入。

这一模式读的是真实文件系统，不是索引，`//` 因此能到达索引范围之外的任何地方。相应地，它能打开的东西也就不止 Markdown，注意别去点 `/dev` 里的东西。
