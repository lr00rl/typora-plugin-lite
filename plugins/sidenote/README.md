...
也可以在 Typora 编辑器中选中文本，点击选区附近浮出的 **Add sidenote**，插件会把选中文本快速转换为：

```html
<span class="sidenote">选中文本</span>
```

同一能力也注册为插件命令 `sidenote:add`，并绑定快捷键 `Mod+Alt+S`（macOS 为 `Cmd+Opt+S`）。没有选区时会插入一个空的 `<span class="sidenote"></span>`。

## 编辑器内效果

- **宽屏 (>=1200px)**：编辑器中的 sidenote 会统一投影到 `#write` 右侧 portal 层，不再参与正文 / 代码块 / 表格的块级排版
- **表格内旁注**：保留表格横向滚动，旁注本体仍然显示在 `#write` 最外层右侧，不再被表格滚动容器裁掉
...
- **桌面宽屏 portal 布局**：编辑器中的 sidenote 在宽屏下会复制一份只读 portal 到 `#write` 右侧边注层，原位置的 sidenote 默认隐藏，编辑时自动回到原文流中
